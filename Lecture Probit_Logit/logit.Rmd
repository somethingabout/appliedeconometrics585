...
When $x$ includes nonlinear functions, such as $age^{2}$, probably makes more sense to use $(\overline{age})^{2}$ rather than average $age_{i}^{2}$. 

For example, you want the partial effect at the average of age where you have a non-linear function, $\beta_1 age_i + \beta_2 age_i^2$
If the average
$\beta_1 age_i + \beta_2 age_i^2$
...
We first estimate a logit or probit model that includes whether the individual received a grant along with controls 
...
Average treatment effect is much more useful (and is the same as the average partial effect)
