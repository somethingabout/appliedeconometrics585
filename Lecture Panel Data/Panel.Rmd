...
#crime2 =  crime2 %>% group_by(d87) %>% mutate(crmrte_diff = crmrte - lag(crmrte,default =0))

#crime2 = crime2 %>% group_by(d87) %>% mutate(cunem_diff = cunem - lag(cunem, default =0)) 
...
#There is a slight issue in R that it doesn't do the correct degress of freedom adjustment for the number of groups. So, we can do this ourselves.
...
In applications where $x_{i t}$ is a dummy variable with no assignment in the first period, the FD estimator has a simple interpretation. It is the same as applying OLS to "
...
At this point, you may have the realization that a Difference-in-Difference model is the same thing as a simple first difference model where there are only two time periods. And that's what the above equation is - the differences are within the same unit. 
...
The treatment effects literature is more about adding observed controls to make *assignment exogenous* and prefers to add $y_{i1}$. The unobserved effects approach is used more by economists but the treatment effects approach is gaining. 

We went over Diff-Diff models last week. The literature in the field is very big. 
Somethings that we won't have time to discuss, but are important to note are:

* ATE vs ATT  [Newey, W. 2007 explainer](https://ocw.mit.edu/courses/economics/14-386-new-econometric-methods-spring-2007/readings/treatment_effect.pdf)
*  time varying treatments by [Callaway and Sant'Anna (2020)](https://arxiv.org/abs/1803.09015). They have an R package called did `install.packages(did)` 
...
If, say, we interact $w_{i t}$ and a variable $r_{i t}$, we construct $w_{i t} r_{i t}$ and then difference it. (Example later.)
...
\end{equation*}$\rightarrow $ $w_{i t}$ is the binary intervention indicator and $\beta $ estimates the average treatment effect of the policy 
...
FE estimation. But $\beta _{0}$ gets eliminated by the FE transformation, so what is the estimated intercept? It is the average of the $N$ coefficients on the dummy variables (rarely of interest).
...
fixed_tw_wage <- plm(lwage~educ+exper+union+married, 
               data = wagepan, 
               index = c("nr", "year"), 
               model = "within", 
               effect = "twoways")
...
fixed_wage_cse <- dfa * vcovHC(fixed_tw_wage, type = "HC0", cluster = "group", adjust = T)
...
In general, we would prefer RE to POLS: $\text{\quad \quad } \rightarrow $ RE removes the fraction $\theta $ of $a_{i}$ from the error term, so it likely has less bias (inconsistency) than POLS  $ \rightarrow $ RE tends to produce more efficient estimators than POLS as it removes at least some, if not all, of the serial
...
The Hausmen test basically tests whether the error term ($u_i$)  is correlated with the regressors ($x_i$). The null hypothesis is they are not correlated (and hence, do not break an assumption of a Random Effects model).

But if RE and FE give estimates that differ in economically important ways, we want evidence that RE should be rejected in favor
of FE. (RE usually gives smaller standard errors; sometimes much smaller. So we would use it if we can.)
...
3. A third caveat concerns the set of parameters that we can compare. Because the FE approach only identiﬁes coeffcients on time-varying explanatory variables, we clearly cannot compare FE and RE coeffcients on time-constant variables. But there is a more subtle issue: we cannot include in our comparison coeffcients on aggregate time effects—that is, variables that change only across t. As with the case of comparing FE and FD estimates, the problem with comparing coefficients on aggregate time effects is not one of identiﬁcation; we know RE and FE both allow inclusion of a full set of time period dummies.


We can estimate models that include aggregate time e¤ects, time-constant variables, and regressors that change across both i and t, by RE and FE estimation. But no matter how we compute a test statistic, we can only compare the coeffcients on the regressors that change across both i and t. 
...
\end{equation*}where $r_{i}$ is a new unobserved effect: $a_{i} =\psi  +\bar{\mathbf{w}}_{i} \mathbf{\xi } +r_{i}$.
...
```
In the above case, the p-value is low, so we reject the null of a Random Effects Model and therefore use a Fixed Effects model.
...
\end{equation*}${\Large  \bullet }$ Here, we are including time averages of all time-varying explanatory variables AND all time-constant variables that appear in the
RE estimation 
...
\hat{\alpha }_{C R E ,t} =\hat{\alpha }_{F E ,t} ,t =1 ,\ldots  ,T\end{gather*}${\Large  \bullet }$ The null hypothesis is that RE is sufficient.  

 This is implemented as $H_{0} :\gamma _{1} =\ldots  =\gamma _{k} =0$ \\\relax ${\Large  \bullet }$ Rejecting this suggests that FE is preferred.  An advantage of CRE is that it allows for estimation of the effects of time-constant explanatory variables, not possible using FE. 

#### Advanced considerations with policy analysis
