...
Loosely, Rj2 “close” to one is called the “problem” of multicollinearity.
...
Also fortunately:
An important fact about independent normal random variables: any linear combination is also normally distributed. Because the ui are independent and identically distributed (iid) as $u ∼ Normal(0,\sigma^2)$,
...
We do not test hypotheses about the estimate! We know what it is once we collect the sample. We hypothesize about the **unknown population value**, $\hat{\beta_j}$.
...
- What a 95% CI means is that for 95% of the random samples that we draw from the population, the interval we compute using the rule $\beta_j ± c \cdot se(\beta_j$ ) will include the value $\beta_j$. But for a particular sample we do not know whether βj is in the interval.
...
$lsalary = β0 + β1years + β2gamesyr + β3bavg +β4hrunsyr + β5rbisyr + u$
...
We can do an F-test! F-tests pull from a different distribution  (the f distirbution) and we can compare our unrestricted model to our restricted model. We want to test if all of our performance variables are jointly statistically significant to zero. If any one is not equal to zero - then we fail to reject the null.
