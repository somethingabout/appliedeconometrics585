...
A great resource that is R for Data Science that is freely available online [here](https://r4ds.had.co.nz/) - I heavily rely on this in the lecture. And another good option YaRrr! the Pirates Guide to R is [here](https://bookdown.org/ndphillips/YaRrr/order-sorting-data.html)
...
If you want to install a package you can do so, like this:
...
In R you use an exclamation mark (!) to exclude something,
...
```{r}
select(jobs, code="_ - code" , rank= "_ - rank")
```
...
#Create another dataframe by education group
```{r}
by_educ <- group_by(jobs, education)
educ_emp <-summarise(by_educ, Nemp = sum(numbEmployed, na.rm=TRUE))
```

# That contains the number of employees by educaiton group
