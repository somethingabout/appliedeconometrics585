...
3. In most cases, taking the log greatly reduces the variable of a variable, making OLS estimates less prone to outlier influence. 
...
\begin{equation}
\log(wage) =\beta_{0} +\beta_{1}faminc +u
\end{equation}

Holding $u$ fixed, 

\begin{equation} 
\Delta \log(salary) =\beta _{1}  \Delta faminc
\end{equation}
...
\begin{equation}100 \beta _{1}  \approx  \%  \Delta bwght\text{when} \Delta faminc =1
\end{equation}
...
We only use the linear approximation (and disregard the terms from the quadratic on wards), it will be more accurate the smaller $\beta$ is. This is fine for small changes, but you may want to report $100*(e^\beta -1) instead$:
...
```{r}
library("wooldridge", "stargazer", "dplyr")
data(bwght)
loglin <- lm(log(bwght) ~ cigs+faminc, data = bwght)
sum_loglin <- summary(loglin)
sum_loglin
 
cigsbeta <- exp(-0.0040326)-1
cigsbeta
faminc <- exp(0.0008437)-1
faminc
```
...
\begin{equation*}\log(bwght) =\beta _{0} +\beta _{1} \log(faminc) +u
\end{equation*}then 

\begin{equation*}\beta_{1} \approx \frac{ \% \Delta bwght}{ \% \Delta faminc}
\end{equation*}
...
The estimated elasticity of CEO salary with respect to firms sales is about .25 or a 1% change in sales is associated with a .257% increase in roe (To interpret this elasticity, we do not need to know $bwght$ is in thousands of dollars while $faminc$ is in millions.) 
...
A 10 percent increase in family income leads to an increase in birth weight by 1.6%

\begin{equation*}.016 (10) =1.6
...
\begin{equation*}bwght) =\beta_{0} +\beta_{1} \log(bwght) +u
\end{equation*}
then 

\begin{equation*}\beta _{1} \approx \frac{ \Delta bwght}{ \Delta log(faminc)}
\end{equation*}
...
For every additional dollar in family income, birth weight will increase by .018 % (1.8/100)

or rather, for every $1,000 increase in birth weight is associated with 18 ounces ((1.8/100)*1000)
...
It is often useful to ask the following question, which allows us to see how important an effect is relative to the population: “How many standard deviations will y change when xj increases by one standard deviation?”
...
library(wooldridge)
data(attend)
attend <- lm(final ~ missed+priGPA+ACT, data = attend)

#using the car package to get the beta coefficients
library(car)
lm.beta(attend)
```

The coefficient on priGPA is larger than than on ACT by almost a factor of five. But does this mean priGPA has the more important effect?

An increase in one sd of priGPA increases final by about .222 sds while a one sd increase in ACT increases final by about .297 sds, which is a larger movement in the distribution of final exam scores.
...
There is only a single explanatory variable, x, but two regressors, $x_1 = x$ and $x_2 = x_2$.

The slope of y with respect to x depends on $β_1$ and $β_2$, and the value of x:
\begin{equation}
\frac{dy}{dx} = \beta_1 + 2\beta_2 x holding u fixed
\end{equation}
Estimation is straightforward. We just define a new variable, $x_2$, and include it along with x as a regressor.

Having estimated the coefficients we write
\begin{equation}
\hat{y} = \hat{\beta_0}+\hat{\beta_1}+\hat{\beta_2}x^2
\end{equation}
...
Because the quadratic model is more complicated to interpret and explain, we should make sure there is good statistical evidence for keeping x2 (exper2) in the equation.
...
Recall that if you have multiple variables that are highly correlated that your inference will change. Well, that's no different here.
...
abline(wage_lin, col = "black", lwd = 2)

# add quadratic function to the plot
order_id <- order(wage1$exper)

lines(x = wage1$exper[order_id], 
      y = fitted(wage_quad)[order_id],
     col = "red", 
    lwd = 2) 
```
...
Holding $x_2$ (and u) fixed, the partial effect of $x_14 on y is:
...
wage2 <-mutate(wage2, mean_adj_educ  = educ-mean_educ, mean_adj_iq = IQ-mean_iq)

#run the regression with the mean adjusted regression
wage_int_meanadj <- lm(log(wage) ~educ + IQ + mean_adj_educ*mean_adj_iq  , data = wage2)

summary(wage_int_meanadj)
...
where μf is the population average wage for women and μm is the popula- tion average wage for men. The t statistic and confidence interval are directly reported.
...
Incidentally, in this data set, gender, marrital status, education, experience, and tenure explain about 45% (using R ̄2) of the variation in lwage.
...
```
Controlling for no other factors, those with below average looks earn about 20.8% less than those with average looks. The t statistic is very significant (p- valueis super low).
...
#### Chow Test 
 ...
 $lwage = β_0 + δ_0female + β_1exper + δ_1female · exper +β_2coll + _δ2female · coll + u$
...
```
 ...
 We can see the different interactions visually:
