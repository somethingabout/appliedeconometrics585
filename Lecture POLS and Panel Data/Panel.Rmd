...
#let's run the simplre regression of the differences between the two time periods on unem on crimerate
...
This is also known as Difference-in-Difference model and it is the same thing as a simple first difference model where there are only two time periods with an interaction. And that's what the above equation is - the differences are within the same unit. 
...
rob_se <- list(sqrt(diag(vcovHC(diff_mult, type = "HC1"))), sqrt(diag(vcovHC(diff_mult_cov, type = "HC1"))),sqrt(diag(vcovHC(diff_mult_wls, type = "HC1"))))

stargazer(diff_mult, diff_mult_cov,diff_mult_wls,
          digits = 3,
          header = FALSE,
          type = "html", 
          se = rob_se,
...
```{r}
#include trends
#diff_mult_trends <- lm(mrate ~ legal+factor(state)*factor(year), data = deaths_all_83)

#summary(diff_mult_trends)

#sqrt(diag(vcovHC(diff_mult_trends, type = "HC1")))
...
Still, propensity scores only make the groups comparable in terms of **observable characteristics** But, you can use diff-in-diff to handle the unobserved heterogeneity that may still exist. 
