...
IV estimator is *consistent*, but not always unbiased.  Bias can be large if the correlation between  $z$ and $x$ is ``small.'' This is something to check for to get a general sense of the size of the bias - this problem is known as a weak instrument (more on this in a moment)
...
\begin{equation}
\hat{\beta_1}^{2SLS} = \frac{s_{\hat{X}Y}}{s^2_\hat{X}} =  \frac{\hat{\pi_1}s_{ZY}}{\hat{\pi}^2_1s^2_Z} = \frac{s_{ZY}}{\hat{\pi_1}s^2_Z}
\end{equation}

Then substituting out $\hat{\pi_1}$ using $\hat{\pi_1} = s_{ZX}/s^2_Z$:
\begin{equation}
\hat{\beta_1}^{2SLS} = \frac{s_{ZY}}{\hat{\pi_1}s^2_Z} = \frac{s_{ZY}}{s_{ZX}}
\end{equation}
...
* Get advice about more sophisticated kinds of IV analysis with weak instruments (such
as Anderson-Rubin test statistics, Anderson-Rubin confidence intervals, LIML - a whole subfield)
...
card_ds<- lm(educ ~ nearc4, card)

coeftest(card_ds, vcov = vcovHC(card_ols, type="HC1"))
```
