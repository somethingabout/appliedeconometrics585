...
If we know what the form of the variance is (as a function of explanatory variables), then Weighted Least Squares (WLS) is more efficient than OLS.  If you know something about the variance, then we can directly model it and incorporate it into our estimation method.
...
ols<- lm(nettfa ~ inc + I(age25^2) + male + e401k, k401ksubs_single )
summary(ols)
...
wls <- lm(nettfa ~ inc + I(age25^2) + male + e401k, k401ksubs_single, weights=wts)
summary(wls)
```
...
```{r, results='asis'}
library(stargazer)
ols_r<-coeftest(ols, vcov = vcovHC(ols, type="HC1"))
wls_r<- coeftest(wls, vcov = vcovHC(wls, type="HC1"))
...
For example, a firm has employees that contribute to a 401k and we only have average values of contributes, earnings and age by employer (we don't have individual level data). Imagine we have the regression model:
...
What we need are these fitted values to then finall estimate $h_i$ of which can then use that stored estimate to implement our WLS.
